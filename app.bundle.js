...
;// CONCATENATED MODULE: ./src/DOMhandler.js
...
;// CONCATENATED MODULE: ./src/todoCreation.js
...
function setupNewTodo() {
    ...
    const todoTab = addTodoTab('My todo', parentElem, undefined, undefined, currProj.id, currTodo.id);
    addTodoTabListeners(todoTab);
    setStorage(parentProj, currProj, currTodo);
    const todoInputs = addTodoInputs();
...
function setupExistingTodos(project) {
    if (project.todoList.length) {
        ...
        project.todoList.forEach(todo => {
            ...
            const todoTab = addTodoTab(text, parentElem, format(new Date(todo.dueDate), 'dd-MM'),
                todo.priority, project.id, todo.id, todo.status);
            addTodoTabListeners(todoTab);
        })
        const currTodo = JSON.parse(localStorage.getItem('current_todo'));
        const todoInputs = addTodoInputs(currTodo.title, currTodo.desc, currTodo.dueDate,
...
function addInputListeners(todoContainer) {
    ...
    if (currProj.todoList.length) {
        ...
        todoContainer.childNodes[5].addEventListener('click', () => {
            const label = addCheckbox(checkboxContainer);
            let id;
...
function checkboxLabelHandler(checkboxContainer, checkboxArr) {
    ...
    checkboxArr.forEach((checkbox, index) => {
        const label = addCheckbox(checkboxContainer, checkbox);
        label.childNodes.forEach(child => {
            child.setAttribute('data', index + 1)
...
function addTodoTabListeners(tab) {
    ...
    removeTab.addEventListener('click', () => {
        ...
        if (currProj.todoList.length > 1 || currProj.id) {
            ...
            setStorage(parentProj, currProj, currTodo);
            const todo = addTodoInputs(currTodo.title, currTodo.desc, currTodo.dueDate,
                currTodo.priority, currTodo.notes, currTodo.checkboxArr, currTodo.status);
            if (currProj.todoList.length) {
                ...
                let todo = addTodoInputs(currTodo.title, currTodo.desc, currTodo.dueDate,
                    currTodo.priority, currTodo.notes, currTodo.checkboxArr, currTodo.status);
                addInputListeners(todo);
            }
            ...
            currTodo = addTodo();
            currProj.todoList.push(currTodo);
            addInputListeners(addTodoInputs());
            currTodo.id = 1;
            tabTitle.value = '';
            setStorage(parentProj, currProj, currTodo);
        }
    ...
    wrapper.addEventListener('click', () => {
        ...
        currTodo = currProj.todoList.at(tab.getAttribute('data')[2] - 1);
        setStorage(parentProj, currProj, currTodo);
        const todo = addTodoInputs(currTodo.title, currTodo.desc, currTodo.dueDate, currTodo.priority,
...
;// CONCATENATED MODULE: ./src/projectCreation.js
...
function initInbox() {
    ...
    inbox.push(currProj);
    projectCreation_setStorage(inbox, currProj)
}
...
if (!localStorage.getItem('inbox_project')) {
    ...
    const inbox = JSON.parse(localStorage.getItem('inbox_project'));
    setupExistingTodos(inbox[0]);
}

function projectCreation_setStorage(parentProject, currProj, currTodo) {
    if (parentProject) {
        if (parentProject[0].id) {
            localStorage.setItem('project_list', JSON.stringify(parentProject));
        } else {
...
function setupNewProject() {
    ...
    projectCreation_setStorage(projList, currProj);
    const projectTab = addProjectTab('My project', currProj.id);
...
function setupExistingProjects() {
    ...
    projList.forEach(project => {
        ...
        const projectTab = addProjectTab(text, project.id);
        //project tab functionality
        addTabListeners(projectTab);
        if (project.todoList.length) {
            setupExistingTodos(project);
        }
    });
}
...
function dropHandler(ev, todoContainer) {
    ...
    if (todoContainerIndex !== currProjIndex) { /* make sure srcParent index isn't the same as destParent index */
        ...
        const todo = addTodoInputs(transferTodo.title, transferTodo.desc, transferTodo.dueDate, transferTodo.priority,
        ...
        projectCreation_setStorage(destParent, destParent.at(todoContainerIndex - 1), destParent.at(todoContainerIndex - 1).todoList.at(-1));
...
function addTabListeners(tab) {
    ...
    removeTab.addEventListener('click', () => {
        ...
        if (!projList.length) {
            ...
            projectCreation_setStorage(inbox, inbox[0], inbox[0].todoList.at(-1));
        } else {
            projectCreation_setStorage(projList, currProj, currProj.todoList.at(-1));
        }
        const projectTabsContainer = document.querySelector('.project-tabs-container');
    ...
    editTab.addEventListener('click', () => {
        ...
        const tabTitleInput = addElement(tabTitle.getAttribute('class'), '', tabTitle.parentNode, 'input');
        ...
        tabTitleInput.addEventListener('blur', () => {
            ...
            const tabTitle = addElement(tabTitleInput.getAttribute('class'), text,
...
function tabClickHandler(tabContainer) {
    ...
    if (currProj.todoList.length) {
        ...
        addInputListeners(addTodoInputs(todo.title, todo.desc, todo.dueDate,
            todo.priority, todo.notes, todo.checkboxArr, todo.status));
    } else {
        addTodoInputs();
    }
    projectCreation_setStorage(parentProj, currProj, currProj.todoList.at(-1));
}
